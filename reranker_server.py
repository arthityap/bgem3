"""
reranker_server.py — Cross-encoder reranking service using BAAI/bge-reranker-v2-m3.
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        _reranker.compute_score(
            [["warmup query", "warmup passage one"], ["warmup query", "warmup passage two"]],
            normalize=True,
        )
...
async def rerank(
    ...
    if len(req.passages) > 100:
        raise HTTPException(status_code=429, detail="max 100 passages per request on M1 16GB")
    ...
    scored = sorted(
        [ScoredPassage(index=i, score=float(s), text=p) for i, (p, s) in enumerate(zip(req.passages, scores))],
