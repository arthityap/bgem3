...
info("Step 1/6: Killing any processes on ports 8000 and 8001...")
kill_port(RAG_PORT)
kill_port(MCP_PORT)
...
info(f"rag_server PID {rag_proc.pid} — waiting for /health (up to {HEALTH_TIMEOUT}s, model load takes ~30s)...")
if not wait_for_health(RAG_URL, HEALTH_TIMEOUT, HEALTH_POLL):
    rag_proc.terminate()
    fail(f"rag_server did not become healthy within {HEALTH_TIMEOUT}s. Check logs/rag_server.log")
ok(f"rag_server healthy on {RAG_URL}")

# ── Step 5: Start mcp_server ─────────────────────────────────────────────────────────
info(f"Step 4/6: Starting mcp_server on port {MCP_PORT}...")
...
# ── Step 6: Smoke tests ───────────────────────────────────────────────────────────────
info("Step 5/6: Running smoke tests (test_service.py)...")
result = subprocess.run([sys.executable, "test_service.py"])
if result.returncode != 0:
    warn("Smoke tests failed — services are running but something is wrong. Check logs/.")
    sys.exit(1)
...
print(f"\nBoth processes are running in the background. Press Ctrl+C to stop both.\n")

# ── Keep alive: wait and forward Ctrl+C to children ────────────────────────────
try:
    rag_proc.wait()
    ...
    rag_proc.wait()
    mcp_proc.wait()
    ok("Both services stopped cleanly.")
